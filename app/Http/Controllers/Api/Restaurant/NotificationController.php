...
class NotificationController extends Controller
{
     ...
     public function register_token(Request $request){
        ...
        return responseJson(1,' token created and logged in',$data);
     }
    ...
    public function remove_token(Request $request){
       ...
       // Token::where('token',$request->token)->delete();
       $data= $request->user()->tokens()->where('token',$request->token)->delete();
        return responseJson(1 , 'token removed and logged out ',$data);
    }
    /*============== notification ============*/
